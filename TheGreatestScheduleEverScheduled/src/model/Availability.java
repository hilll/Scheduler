 ...
 * Availability
 * 
 * Employees each have an availability. Stand-alone Availabilities should not be
 * created, when a new employee is created or loaded, the Availability
...
public class Availability {
	...
	// array of lists (one for each day of the week) of TimeBlocks employee is
	// available to work
	...
	// list of time blocks during which employee has stated he/she is
	// unavailable
	...
	/**
	 * Arguments: availability: should be null for new Availability object,
	 * actual String[] of length 7 for loaded one. isMananger: whether the
	 * availability belongs to a manager or not.
	 */
	...
	private void fillUnavailabilityAndAvailabilitiesByDay(int day) {
		...
		int streakType = -1; // 1 if currently counting available slots, 0
								// otherwise
		...
		for (int c = 0; c < daysAvailability.length(); c++) {
			...
			if (streakType != curr) { // either start of string, or switching
										// from avail to unavail, or from
										// unavail to avail
				if (curr == 1) { // last streak was of 0s, or it's the start of
									// the string
					if (c == 0) {
						availableStart = 0;
				...
				} else { // last streak was of 1s, or it's the start of the
							// string
					if (c == 0) {
						unavailableStart = 0;
					} else { // not the beginning of string
	 ...
	 * HashMap<String, TimeSlot> slots = new HashMap<String, TimeSlot>(); for
	 * (TimeSlot ts : unavailabilityList) { slots.put(ts.toString(), ts); }
	 * return slots; }
	 */
	...
	/**
	 * Changes the value of an availability string. Inputs: day: which string to
	 * change, start and end: segment of the availability string to change,
	 * availability: what to change the segment to. Does not check to make sure
	 * a change is actually being made.
	 */
	...
	public void fillAvailabilityPool(Schedule company) {
		...
		for (int day = 0; day < this.availabilityByDay.size(); day++) { // time
																		// blocks
																		// this
																		// has
			for (TimeSlot thisSlot : this.availabilityByDay.get(day)) {
				for (TimeSlot thatSlot : company.getShiftsByDay().get(day)) { // time
																				// blocks
																				// that
																				// has
					if (thisSlot.canFit(thatSlot)) {
						this.availabilityPool.add(thatSlot); // Put shift in emp
																// pool
	...
	/**
	 * Deletes the Availability that belongs to the Employee with empID from the
	 * Database.
	 */
	...
	/**
	 * Saves or updates the Availability that belongs to the Employee with empID
	 * in the Database.
	 */
	public boolean save(int empID) {
		...
		if (Database.tableContainsID(getTableName(), empID)) {
			return Database.executeManipulateDataQuery(String.format(
					"UPDATE `%s`.`%s` SET `sunday`='%s',`monday`='%s',`tuesday`='%s',`wednesday`='%s',`thursday`='%s',`friday`='%s',`saturday`='%s'"
							+ " WHERE `emp_id`=%d",
					Database.getName(), getTableName(), availabilityStrings[0], availabilityStrings[1],
					availabilityStrings[2], availabilityStrings[3], availabilityStrings[4], availabilityStrings[5],
					availabilityStrings[6], empID));
		}

		// INSERT
		return Database.executeManipulateDataQuery(String.format(
				"INSERT INTO `%s`.`%s` "
						+ "(`emp_id`, `sunday`, `monday`, `tuesday`, `wednesday`, `thursday`, `friday`, `saturday`)"
						+ " VALUES (%d, '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
				Database.getName(), getTableName(), empID, availabilityStrings[0], availabilityStrings[1],
				availabilityStrings[2], availabilityStrings[3], availabilityStrings[4], availabilityStrings[5],
				availabilityStrings[6]));
	}
